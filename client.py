...
class Client:
    ...
    def send_logout_info(self, username, password):
        """
        发送登录用户的用户名和密码给服务器验证，并return验证结果
        :param username: 待验证的用户名
        :param password: 待验证的密码
    ...
    def send_register_info(self, username, password, confirm, check):
        """
        ...
        username_psw = username + "#!#!" + password + "#!#!" + check
        self.client_socket.sendall(username_psw.encode("utf-8"))
    ...
    def send_repsd_info(self, username, password, confirm, check):
        """
        发送用户注册的用户名和密码给服务器，并返回注册结果
        :param check: 验证信息
        :param username: 待注册的用户名
...
class LoginPanel:
    ...
    def login_logout(self):
        ...
        username, password = self.get_input()
        client = Client()
        check_result = client.send_logout_info(username, password)
        if check_result == "用户名不存在，或密码错误，无法注销账号！":
            messagebox.showerror(title="Error", message="用户名不存在，或密码错误，无法注销账号！")
        elif check_result == "注销成功！":
            messagebox.showinfo(title="Success", message="注销成功！")
...
class RegPanel:
    ...
    def get_input(self):
        ...
        """
        return self.username.get(), self.password.get(), self.confirm.get(), self.check.get()
    ...
    def repsd_func(self):
        username, password, confirm, check = self.get_input()
        client = Client()
        ret = client.send_repsd_info(username, password, confirm, check)
        print(ret)
        if ret == "密码不一致，请重新输入！":
            messagebox.showerror(title="Error", message="密码不一致，请重新输入！")
        else:
            if ret == "用户不存在或密保有误":
                messagebox.showerror(title="Error", message="用户不存在或密保有误")
            elif ret == "修改密码成功":
                # 注册成功后提示，然后跳回登录界面
                messagebox.showinfo(title="Success", message="修改密码成功")
                self.close()
                login_panel = LoginPanel()
                login_panel.show()
            else:
    ...
    def reg_func(self):
        ...
        username, password, confirm, check = self.get_input()
        client = Client()
        ret = client.send_register_info(username, password, confirm, check)
        print(ret)
        if ret == "密码不一致，请重新输入！":
            messagebox.showerror(title="Error", message="密码不一致，请重新输入！")
        else:
...
class MainPanel:
            ...
            try:
                ...
                if recv_data:
                    ...
                    if option == "#!onlines#!":
                        ...
                        print("获取历史消息")
                        history = ret.group(2)  # 将历史消息提取出来
                        his = re.match(r"(.*)#!([\s\S]*)#!([\s\S]*)", history)
                        username = his.group(1)
                        datetime = his.group(2)
                        content = his.group(3)
                        self.show_history(username, datetime, content)
    ...
    def show_history(self, username, datetime, content):
        """
        将接受到的消息显示在消息文本框中，自己的消息用蓝色的，别人的消息用黑色
        :param username: 发送消息的用户的用户名
        :param content: 待显示的消息内容
